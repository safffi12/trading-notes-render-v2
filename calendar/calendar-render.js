(function () {
  ...
  function getNextEvent(item) {
    if (item && Array.isArray(item.upcomingEvents) && item.upcomingEvents.length > 0) {
      return item.upcomingEvents[0];
    }

    if (item && item.nextRelease) {
      return {
  ...
  function renderOverviewMetrics(item) {
    const metrics = getFirstMetrics(item, 3);
  ...
  function renderDetailPage() {
    ...
    if (!data) {
      detailContainer.innerHTML = `
        <a href="calendar-page.html" class="back-link">← Назад к календарю</a>
        ${renderErrorCard(
          "Карточка не загрузилась",
          "Файл calendar-data.js не подключился или содержит синтаксическую ошибку."
        )}
      `;
      return;
    }
